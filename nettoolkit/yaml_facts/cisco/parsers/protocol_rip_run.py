...
class RIPConf(ProtocolsConfig):
	...
	def _get_attributes(self, lines):
		...
		for line in lines:
			line = line.strip()
			spl  = line.split()
			self._get_rip_version(attr_dict, line, spl)
			self._get_auto_summary(attr_dict, line, spl)
			self._get_source_validation(attr_dict, line, spl)
			self._get_networks(attr_dict, line, spl)
			self._get_neighbors(attr_dict, line, spl)
			self._get_redistributions(attr_dict, line, spl)
			self._get_default_metric(attr_dict, line, spl)
			self._get_offset_list(attr_dict, line, spl)
		return attr_dict		

	# ---------------------------------------------------------------------- #

	def _get_rip_version(self, attr_dict, line, spl):
		if spl[0] == 'version':
			available_versions = spl[1:]
			append_attribute(attr_dict, 'version', available_versions)

	def _get_auto_summary(self, attr_dict, line, spl):
		if line.strip() == 'no auto summary':
			append_attribute(attr_dict, 'auto-summary', False)

	def _get_source_validation(self, attr_dict, line, spl):
		if 'validate-update-source' in spl:
			append_attribute(attr_dict, 'validate-update-source', spl[0] != 'no')

	def _get_networks(self, attr_dict, line, spl):
		if spl[0] == 'network':
			if len(spl)>2:
...
def get_rip_running(command_output):
	...
	if RC.rip_vrf_dict:
		return {'protocols': {'rip': {'instances': RC.rip_vrf_dict}} }
	else:
		return {'protocols': {}}
