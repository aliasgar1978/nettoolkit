...
from .fstr import standardize_if, if_standardize
from .flist import list_variants
...
def nslookup(ip):
	...
	"""	
	lst = os.popen(f"nslookup {ip}").read().split("\n")
	for line in lst:
...
class IP():
	...
	def ping_average(ip):
		...
		"""		
		lst = os.popen(f"ping {ip}").read().split("\n")
		for x in lst:
			if "Average" in x:
				avg = x.split()[-1]
				s = ''
...
def get_vlans_cisco(line):
	...
	if line.startswith("switchport trunk allowed"):
		vlans['vlan_members'] = list_variants(trunk_vlans_cisco(line))['csv_list']
