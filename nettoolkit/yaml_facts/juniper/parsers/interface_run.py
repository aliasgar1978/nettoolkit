...
class RunningInterfaces(Running):
	...
	def interface_read(self, func):
		...
		for l in self.set_cmd_op:
			...
			if not p: continue
			port_dict = get_int_port_dict(op_dict=ports_dict, port=p)
		...
		for l in self.set_cmd_op:
			...
			try:
				...
				if spl[3] == 'interface':
					...
					vrf = spl[2]
					intf = spl[-1]
					port_dict = get_int_port_dict(op_dict=self.interface_dict, port=intf)
					port_dict['vrf'] = vrf
					foundavrf = True
			except:
				continue
	...
	def get_int_vlan_details(self, port_dict, l, spl, p):
		...
		# key = 'access_vlan'
		if port_dict.get('interface_mode') and self.port_dict['interface_mode'] == 'trunk':
			key = 'vlan_members'
		elif port_dict.get('interface_mode') and port_dict['interface_mode'] == 'access':
			key = 'access_vlan'
		else:
			return None
	...
	def get_interface_mode(port_dict, l, spl, p):
		...
		if not port_dict.get('interface_mode'): 
	...
	def get_interface_channel_grp(port_dict, l, spl, p):
		...
		grp = ''
		if spl[-2] == "802.3ad":
			if not port_dict.get("port_channel"): 
				port_dict['port_channel'] = {}
			grp = spl[-1][2:]
			port_dict['port_channel']['grp'] = grp
			port_dict['port_channel']['interface'] = spl[-1]
		return port_dict
	...
	def int_dot_zero_merge_to_parent1(self):
		...
		"""
		for int_type, int_dict in self.interface_dict.items():
			parents = set()
			for intf, intf_dic in int_dict.copy().items():
				if isinstance(intf, int) or not intf.endswith(".0"): continue
				parent = intf.split(".")[0]
				if parent in int_dict.keys():
					int_dict[parent].update(intf_dic)
					del(int_dict[intf])
			...
			for k, v in int_dict.items():
				if int_type == 'physical':
					...
					try:
						if isinstance(k, int):
							v['int_number'] = k
							v['logical_int_number'] = 0
						else:
							for i, s in enumerate(k):
								if s.isdigit(): break
							v['int_number'] = int(str(k[i:]).split(".")[0])
							v['logical_int_number'] = get_logical_port_number(k)
					except: pass
...
def get_logical_port_number(port):
	...
	if len(spl_port_sc) > 1:
		return int(spl_port_sc[-1])
	if len(spl_port_dot) > 1:
		return int(spl_port_dot[-1])
	return 0
