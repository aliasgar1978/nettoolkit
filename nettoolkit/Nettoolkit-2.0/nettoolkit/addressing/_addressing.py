...
from nettoolkit.cmn.fstr import string_concate, found, is_blank_line, is_hostname_line, replace_dual_and_split, find_any
from nettoolkit.cmn.flist import list_to_octet
...
def expand(v6subnet, withMask=False):
	...
	if len(sip) == 2:
		...
		for x in range(1, 9):
			p = string_concate(p, get_hext(v6subnet, hexTnum=x), conj=':')
		v6subnet = p
	else :
		# ~~~~~~~ pad leading zeros ~~~~~~~
		lsip = sip[0].split(":")
		for x in range(8-len(lsip), 0, -1):
			p = string_concate(p, '0', conj=":")
		if p != '':
			v6subnet = p + ':' + v6subnet
...
def binsubnet(subnet):
	...
	"""    	
	try:
		if found(subnet, "."): version, split_by, bit_per_oct = 4, ".", 8
		if found(subnet, ":"): version, split_by, bit_per_oct = 6, ":", 16
		s = ''
...
def get_classful_cidr(ip_str):
    """
    Returns a classful CIDR suffix string if no explicit mask is provided.
    """
    try:
        ...
        pass
    return "255.255.255.255"         # Host fallback safety guard
...
class IPv6(IP):
	...
	def _get_hextates(self, hxts=1, s=''):
		...
		for o in range(1, hxts+1):
			ox = string_concate(ox, self._get_hext(o, s=s), conj=':')
		return ox+":"
...
class IPv4(IP):
	...
	def subnet_zero(self, withMask=True):
		...
		try:
			...
			bm = self._binmask
			net = list_to_octet(self._octets_bin2dec(self._bin_and(s, bm )))
			if withMask :
	...
	def broadcast_address(self, withMask=False):
		...
		try:
			s = binsubnet(self.subnet)
			im = self._invmask
			bc = list_to_octet(self._octets_bin2dec(self._bin_or(s, im )))
			if withMask :
	...
	def n_thIP(self, n=0, withMask=False, _='', summary_calc=False):
		...
		if (any([addedbin > binsubnet(self.broadcast_address()), 
			...
			ip = list_to_octet(self._octets_bin2dec(addedbin))
			if withMask :
	...
	def binmask(self):
		...
		return list_to_octet(self._octets_bin2dec(self._binmask))

	@property
	def invmask(self):
		'''Inverse Mask from provided IP/Subnet'''
		return list_to_octet(self._octets_bin2dec(self._invmask))
...
class Routes(object):
	...
	def __init__(self, hostname, route_list=None, route_file=None):
		...
		"""    		
		if route_file != None: route_list = file_to_list(route_file)
	...
	def __parse(self, route_list, hostname):
		...
		for line in route_list:
			...
			if is_hostname_line(line, hostname): continue
