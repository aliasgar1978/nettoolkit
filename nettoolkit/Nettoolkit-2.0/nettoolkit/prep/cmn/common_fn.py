...
from nettoolkit.cmn.flist import convert_vlans_list_to_range_of_vlans_list, split_to_group, expand_vlan_list, remove_empty_members
...
def list_append(lst, item):
    ...
    Returns:
        list: updated list
    """	
    current_list = list(lst) if isinstance(lst, (list, tuple, set)) else [lst]
...
def list_extend(lst, item):
    ...
    """
    current_list = list(lst) if isinstance(lst, (list, tuple, set)) else [lst]
    extension_items = item if isinstance(item, (list, tuple, set)) else [item]
    current_list.extend(extension_items)
    return current_list
...
def groups_of_nine(lst):
    ...
    """	
    lst = convert_vlans_list_to_range_of_vlans_list(lst)
    lst = [ str(_) for _ in lst ]
    return split_to_group(lst, 9)	
...
def physical_if_allowed(vlan, table):
    ...
    for key, data in table.items():
        if data['filter'].lower()=='physical' and int(vlan) in expand_vlan_list(str_to_list(data['vlan_members'])):
...
def nth_ip(net, n, withMask=False):
    ...
    """	
    try:
        interface_obj = ip.ip_interface(net)
        _net = str(interface_obj.network)
        v4 = ip_addressing(_net)
        
        if withMask:
            # Re-append the original network configuration mask context cleanly
            allocated_ip = v4.n_thIP(n, False)
            return f"{allocated_ip}/{interface_obj.network.prefixlen}"
        return v4[n]
    except Exception:
        return ""
...
def mask(net):
    ...
    if net:
        try:
            return str(ip.ip_interface(net).network.prefixlen)
        except Exception:
            return 'n/a'
    return 'n/a'
...
def invmask(net):
    ...
    """	
    try:
        return str(ip.ip_interface(net).with_netmask.split('/')[-1])
    except Exception:
        try:
            # Safe boundary fallback using your internal IPv4 class objects if required
            v4 = v4addressing(net)
...
def get_summaries(lst_of_pfxs):
    ...
    """	
    lst_of_pfxs = remove_empty_members(lst_of_pfxs)
    try:
...
def get_item(lst, n):
    ...
    try:
        if isinstance(lst, (list, tuple)):
            return lst[n]
        return lst  # If input is a raw string/regex, return the whole element cleanly!
    except Exception:
        return lst
