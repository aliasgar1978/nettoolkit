...
class RunningInterfaces():
	...
	def interface_read(self, func):
		...
		for l in self.cmd_op:
			...
			if l.startswith("interface "):
				...
				if not ports_dict.get(int_filter):
					ports_dict[int_filter] = {}
				int_filter_dict = ports_dict[int_filter]
				#
	...
	def get_int_channel_group(port_dict, l):
		...
		port_dict['port_channel']['interface'] = "Port-channel" + po
		port_dict['port_channel']['mode'] = po_mode
		port_dict['port_channel']['group'] = po
	...
	def get_interface_v4_helpers(port_dict, l):
		...
		if l.strip().startswith("ip helper-address"):
			if not port_dict.get('v4_helpers'):
				port_dict['v4_helpers'] = set()
			port_dict['v4_helpers'].add(l.strip().split()[-1])
