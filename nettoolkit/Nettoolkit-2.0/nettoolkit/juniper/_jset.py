...
from nettoolkit.cmn.fstr import delete_trailing_remarks
# ---------------------------------------------------------------------------- 
# J-Set
# ----------------------------------------------------------------------------
@dataclass
class JSet():
    ...
    def _validate_input_list(self):
        ...
        if not isinstance(self.input_list, (list, tuple)):
            raise Exception(f"[-] {self.device}: InputListReadError: must be of type either list or tuple, got {type(self.input_list)}")
    ...
    def convert_to_set(self):
        ...
        for line in self.input_list:
            ...
            # Intercept and process hostnames cleanly without quote artifacts
            if any(stripped_line.startswith(h) for h in self.hostname_str):
                self.check_n_update_hostname(stripped_line)

            stripped_line = delete_trailing_remarks(stripped_line) 		       # Remove Trailing remarks				
            ...
            # Only process multi-line lists if we are not evaluating an inline regex expression string
            # ==================================================================
            # is_regex_line = '"' in stripped_line and stripped_line.count('"') >= 2
            # if not is_regex_line or multiline_string_prev_line:
            ...
            # ==================================================================
            # STATE MACHINE ENGINE: Process bracket hierarchies contextually
            # ==================================================================

            # Pull the parent name segment token and push it onto the path trace stack
            if self.is_brckt_start(stripped_line):
                bt_item_in_orders.append(left_str)
                left_str += " " + stripped_line[:-1].strip() 
                continue

            # Pop back up one hierarchy layer when hitting a closing bracket
    ...
    def check_n_update_hostname(self, line):
        try:
            tokens = line.split()
            raw_host = tokens[-1].replace(";", "").strip().strip('"')
            if raw_host and raw_host != self.device:
                self.send_to_conversion_log(f"[-] updated hostname: [{self.device}] >> [{raw_host}]")
                self.device = raw_host
        except Exception:
            pass
    ...
    def remove_remarks_from_config(self):
        ...
        for line in self.input_list:
            stripped_line = delete_trailing_remarks(line.strip())
